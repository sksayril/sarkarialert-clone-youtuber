...
const AdmitCardPage: React.FC = () => {
  ...
  useEffect(() => {
    fetch("https://7cvccltb-3110.inc1.devtunnels.ms/latest-jobs/public/AdmitCards")
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          setItems(data.data);
        } else {
          setError("No admit card data found");
        }
        setLoading(false);
      })
      .catch(() => {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <h1 className="text-2xl font-bold mb-6 text-center">Admit Card</h1>
